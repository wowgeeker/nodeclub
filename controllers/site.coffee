...
exports.index = (req, res, next) ->
  ...
  keyword = req.query.q or "" # in-site search
  keyword = keyword.join(" ")  if Array.isArray(keyword)
  keyword = keyword.trim()
  limit = config.list_topic_count
  render = (tags, topics, hot_topics, stars, tops, no_reply_topics, pages) ->
    all_tags = tags.slice(0)
    
    # 计算最热标签
    tags.sort (tag_a, tag_b) ->
      tag_b.topic_count - tag_a.topic_count

    
    # 计算最新标签
    tags.sort (tag_a, tag_b) ->
      tag_b.create_at - tag_a.create_at
